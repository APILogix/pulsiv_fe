...
export function MetricLoader({ label = "Loading metric data", detail = "Synchronizing telemetry windows", className }: { label?: string; detail?: string; className?: string }) {
  return (
    <div className={cn("metric-loader flex min-h-[320px] w-full flex-col items-center justify-center rounded-[var(--radius-lg)] border border-[var(--border)] bg-[var(--bg1)] p-6", className)} role="status" aria-live="polite" aria-busy="true">
      <svg viewBox="0 0 344 144" className="h-auto w-full max-w-[344px]" aria-hidden="true">
        ...
        {/* §3 — chart fills are flat low-alpha, never a gradient. */}
        <path d="M16 116 C50 110 60 82 94 88 S140 104 168 70 S220 84 244 48 S286 66 328 28 L328 128 L16 128 Z" fill="var(--brand)" fillOpacity="0.14" />
      ...
      <p className="mt-5 font-[family-name:var(--mono)] text-[10px] font-medium uppercase tracking-[0.09em] text-[var(--text2)]">{label}</p>
