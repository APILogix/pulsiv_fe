...
export function FeaturesTab({ features, onChange }: FeaturesTabProps) {
  const coreFeatures = [
    {
      ...
      tooltip: 'Captures unhandled exceptions, unhandled promise rejections, and manually reported errors. Sends them to Pulse with stack traces, breadcrumbs, and context.',
    ...
    },
    {
      ...
      tooltip: 'Generates trace spans for incoming requests and outgoing calls, enabling you to see latency breakdowns across services.',
    ...
    },
    {
      ...
      tooltip: 'Captures HTTP request metadata (method, URL, status code, duration) for every inbound and outbound request. Does NOT capture bodies or headers unless enabled in Privacy.',
  ...
  const advancedFeatures = [
    {
      ...
      tooltip: 'Tracks scheduled job executions (cron, BullMQ repeatable, etc.) and alerts you if a job misses its expected check-in window.',
    ...
    },
    {
      ...
      label: 'Profiling',
      tooltip: 'Collects CPU and memory profiles of your running process. Beta feature — adds ~2% CPU overhead.',
    ...
    },
    {
      ...
      tooltip: 'Measures event loop lag (the delay between when a callback is scheduled and when it actually runs). High lag = blocked thread.',
      onDesc: 'Event loop lag is measured.',
      offDesc: 'Event loop monitoring is disabled.',
    },
  ];

  const renderToggle = ({ key, label, tooltip, onDesc, offDesc }: any) => {
    const isActive = features[key as keyof typeof features];
    return (
  ...
  return (
    <div className="animate-in fade-in duration-300">
      <SectionBanner
        title="Features"
        definition="These toggles control the high-level subsystems of the SDK. Turning a feature off completely unloads its code path, eliminating any performance overhead. Note that some features like Profiling or Session Replay may have non-trivial CPU/Memory costs."
      >
        Enable or disable core observability pillars and advanced diagnostics. Disabling a feature entirely removes its overhead from your application.
      </SectionBanner>
      ...
      <div className="grid gap-6 md:grid-cols-2">
        ...
        <div className="rounded-[12px] border border-[var(--border)] bg-[var(--bg1)] shadow-sm">
          ...
          <div className="divide-y divide-[var(--border)] px-5">
            {advancedFeatures.map(renderToggle)}
          </div>
        </div>
      </div>
    </div>
