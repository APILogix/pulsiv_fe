...
import { PageHeader, FillPage, Timestamp } from "@/shared/observe";
...
export default function IntegrationsAuditPage() {
  ...
  return (
    <FillPage>
      <PageHeader 
        title="Integration Audit Logs" 
        description="Global audit trail of configuration changes and lifecycle events across all connectors in your workspace." 
      />

      <div className="flex flex-col gap-6 max-w-5xl animate-in fade-in slide-in-from-bottom-2 duration-300">
        <Card>
          <CardContent className="pt-6">
            {isLoading && page === 0 ? (
              ...
              <div className="py-8 text-center text-[var(--text2)] border border-dashed border-[var(--border)] rounded-xl bg-[var(--bg2)]/50">
                ...
                <p className="text-xs mt-1">When integrations are added, configured, or removed, they will appear here.</p>
              </div>
            ) : (
                  ...
                  <table className="w-full text-sm">
                    <thead className="bg-[var(--bg2)] border-b border-[var(--border)]">
                      <tr>
                        ...
                        <th className="h-10 px-4 text-left font-medium text-[var(--text2)]">Target Connector</th>
                    ...
                    <tbody>
                      {logs.map((log: any) => (
                        <tr key={log.id} className="border-b border-[var(--border)] last:border-0 hover:bg-[var(--bg2)]/50 transition-colors">
                          ...
                          <td className="p-4 text-[var(--text2)]">
                            {log.connectorId ? (
                              <span className="font-mono text-xs truncate max-w-[120px] inline-block" title={log.connectorId}>
                                {log.connectorId}
                              </span>
                            ) : (
                              <span className="italic text-xs">Global</span>
                            )}
                          </td>
