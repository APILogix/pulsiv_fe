  ...
  return (
    <div className="flex w-full max-w-[1000px] flex-col gap-6 animate-in fade-in duration-300">
      <div className="flex items-start justify-between gap-4">
        <div>
          <h1 className="mb-2 text-[24px] font-semibold text-foreground tracking-normal">Trusted devices</h1>
      ...
      <div className="overflow-hidden rounded-lg border border-border bg-card">
        <div className="grid grid-cols-[2fr_1.4fr_1.4fr_120px] border-b border-border px-6 py-4 text-[13px] font-medium text-muted-foreground">
          <div>Device</div>
          <div>Trusted</div>
          <div>Expires</div>
          <div>Action</div>
        ...
        {isLoading ? (
          ...
          devices.map((device, index) => (
            <div
              key={device.id}
              className={`grid grid-cols-[2fr_1.4fr_1.4fr_120px] items-center px-6 py-5 hover:bg-white/[0.02] ${index !== devices.length - 1 ? 'border-b border-border' : ''}`}
            >
              <div className="flex min-w-0 items-center gap-4">
                <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-md border border-input bg-background text-muted-foreground">
                  <Laptop size={18} />
                </div>
                <div className="min-w-0">
                  <div className="flex items-center gap-2">
                    <p className="truncate text-[14px] font-medium text-foreground">{device.device_name || 'Trusted device'}</p>
                    <Badge className="bg-[#10b981]/10 text-primary">Trusted</Badge>
                  </div>
                  <p className="mt-1 truncate font-mono text-[11px] text-muted-foreground">{device.id}</p>
                </div>
              </div>
              <div className="text-[13px] text-muted-foreground">{formatDate(device.trusted_at)}</div>
              <div className="text-[13px] text-muted-foreground">{formatDate(device.expires_at)}</div>
              <div>
                <Button
                  variant="outline"
                  size="sm"
                  ...
                  className="border-border bg-[#1a1a1a] text-foreground hover:border-[#ef4444]/30 hover:text-[#ef4444]"
                >
