...
import {
  PageHeader, FillPage, SectionCard, InfiniteTable, StatusBadge, Field, SubmitButton, inputClass, Timestamp, FilterBar, FilterSelect
} from "@/shared/observe";
...
export default function InvitationsPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: [...orgQueryKeys.invitations(activeOrgId!), status],
    queryFn: () => orgApi.listInvitations(activeOrgId!, { limit: 100, status: status || undefined }),
    enabled: !!activeOrgId,
  });
  ...
  return (
    <FillPage className="gap-5">
      ...
      <InfiniteTable
        ...
        loading={isLoading}
        items={invites}
        queryKey={["invitations-table", activeOrgId, status]}
        columns={columns}
        getKey={(i) => i.id}
      />
    </FillPage>
