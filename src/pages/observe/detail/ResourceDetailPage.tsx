...
function ResourceSpecificSections({ resource, entity, detail }: { resource: DetailResource; entity: JsonRecord; detail: ObservabilityEventDetail }) {
  ...
  if (resource === "requests") {
    ...
    return <div className="flex flex-col gap-4"><SectionCard title="HTTP information"><FieldGrid fields={RESOURCE_FIELDS.requests.slice(0, 5)} entity={entity} columns={2} /></SectionCard><SectionCard title="Client and device"><FieldGrid fields={RESOURCE_FIELDS.requests.slice(9)} entity={entity} columns={3} /></SectionCard><JsonPayloadSection title="Headers" value={headers} /><JsonPayloadSection title="Query parameters" value={query} /><JsonPayloadSection title="Path parameters" value={path} /><JsonPayloadSection title="Request body" value={body} /><JsonPayloadSection title="Response body" value={response} /></div>;
  }

  if (resource === "traces") return <div className="flex flex-col gap-4"><SectionCard title="Trace overview"><FieldGrid fields={RESOURCE_FIELDS.traces} entity={entity} columns={3} /></SectionCard><SpanTreePanel detail={detail} /></div>;
  if (resource === "spans") return <div className="flex flex-col gap-4"><SectionCard title="Span overview"><FieldGrid fields={RESOURCE_FIELDS.spans} entity={entity} columns={3} /></SectionCard><SpanTreePanel detail={detail} /></div>;
  if (resource === "logs") return <div className="flex flex-col gap-4"><SectionCard title="Log message"><div className="whitespace-pre-wrap break-words font-[family-name:var(--mono)] text-[13px] leading-relaxed text-[var(--text)]">{stringValue(pick(entity, ["message"])) ?? "No message returned."}</div></SectionCard><SectionCard title="Log context"><FieldGrid fields={RESOURCE_FIELDS.logs} entity={entity} columns={3} /></SectionCard></div>;
  if (resource === "metrics") return <div className="flex flex-col gap-4"><SectionCard title="Metric statistics"><FieldGrid fields={RESOURCE_FIELDS.metrics} entity={entity} columns={3} /></SectionCard></div>;
  if (resource === "profiles") return <div className="flex flex-col gap-4"><SectionCard title="Profile summary"><FieldGrid fields={RESOURCE_FIELDS.profiles} entity={entity} columns={3} /></SectionCard><ProfileSamples detail={detail} /></div>;
  return <div className="flex flex-col gap-4"><SectionCard title="Cron execution"><FieldGrid fields={RESOURCE_FIELDS.crons} entity={entity} columns={3} /></SectionCard><CronTimeline entity={entity} /></div>;
}
...
export default function ResourceDetailPage({ resource, id }: { resource: DetailResource; id: string }) {
  ...
  const tabs = useMemo(() => detail && entity ? [
    { id: "investigation", label: "Investigation", content: <div className="flex flex-col gap-4"><ResourceSpecificSections resource={resource} entity={entity} detail={detail} /><TimelinePanel timeline={detail.timeline} /><StructuredSections detail={detail} /></div> },
  ...
  return <main className="flex flex-col gap-5 pb-8"><Button variant="ghost" className="w-fit" onClick={() => navigate(-1)}><ArrowLeft className="size-4" />Back to {config.label.toLowerCase()}</Button><InvestigationHeader resource={resource} entity={entity} detail={detail} id={canonicalId} /><OverviewCards entity={entity} /><div className="grid items-start gap-5 lg:grid-cols-[minmax(0,1fr)_320px]"><article className="min-w-0"><Tabs tabs={tabs} /></article><MetadataSidebar resource={resource} entity={entity} detail={detail} /></div></main>;
}
