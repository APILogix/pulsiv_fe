...
import {
  PageHeader, KpiCard, FillPage, InfiniteTable, StatusBadge, Button, FilterBar, FilterSelect, Timestamp
} from "@/shared/observe";
...
export default function MembersPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: [...orgQueryKeys.members(activeOrgId!), status, role],
    queryFn: () => orgApi.listMembers(activeOrgId!, { limit: 100, status: status || undefined, role: role || undefined }),
    enabled: !!activeOrgId,
  });
  ...
  return (
    <FillPage className="gap-5">
      ...
      <FilterBar onClear={() => { setStatus(""); setRole(""); }}>
        <FilterSelect
          label="Status"
          value={status}
          onChange={setStatus}
      ...
      <InfiniteTable
        ...
        loading={isLoading}
        items={members}
        queryKey={["members-table", activeOrgId, status, role]}
        columns={columns}
        getKey={(m) => m.id}
        onRowClick={(m) => navigate(`/admin/members/${m.userId}`)}
      />
    </FillPage>
  );
}
