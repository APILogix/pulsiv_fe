  ...
  return (
    ...
    >
      ...
      <SectionCard title="Performance & Features">
        {isPro && <Toggle label="Custom Metrics Collection" description="Enable ingestion of custom business metrics." checked={localConfig?.features?.metrics ?? false} onChange={(v: boolean) => updateFeature("metrics", v)} badge={!isEnt ? "Pro" : undefined} />}
        {isEnt && <Toggle label="CPU/Memory Profiling" description="Continuous profiling in production." checked={localConfig?.features?.profiling ?? false} onChange={(v: boolean) => updateFeature("profiling", v)} badge="Enterprise" />}
        {isEnt && <Toggle label="Session Replay" description="Capture DOM changes and user interactions." checked={localConfig?.features?.sessionReplay ?? false} onChange={(v: boolean) => updateFeature("sessionReplay", v)} badge="Enterprise" />}
        {!isPro && <div className="text-sm text-[var(--text3)] italic py-2">Advanced performance monitoring features require a Pro or Enterprise plan.</div>}
      </SectionCard>
      ...
      <SectionCard title="Privacy & Scrubbing">
        <Field label="Scrubbed Headers" hint="Comma-separated list of HTTP headers to mask.">
          <input className={inputClass} value={(localConfig?.privacy?.scrubbing?.headers || []).join(", ")} onChange={(e) => updatePrivacy("scrubbing", { ...(localConfig?.privacy?.scrubbing || {}), headers: e.target.value.split(",").map((s: string) => s.trim()).filter(Boolean) })} placeholder="authorization, cookie, x-api-key" />
        </Field>
      </SectionCard>

      {(isPro || isEnt) && (
        <SectionCard title="Instrumentation">
          {isPro && <Toggle label="MongoDB Driver" description="Auto-instrument MongoDB queries." checked={localConfig?.instrumentation?.mongodb ?? false} onChange={(v: boolean) => updateInstrumentation("mongodb", v)} badge={!isEnt ? "Pro" : undefined} />}
          {isEnt && <Toggle label="Redis Driver" description="Auto-instrument Redis commands." checked={localConfig?.instrumentation?.redis ?? false} onChange={(v: boolean) => updateInstrumentation("redis", v)} badge="Enterprise" />}
        </SectionCard>
      )}
