...
export function KillswitchesTab({ killswitches, onChange }: KillswitchesTabProps) {
  ...
  return (
    <div className="animate-in fade-in duration-300">
      <SectionBanner title="Danger Zone" type="danger">
      ...
      <div className="rounded-[12px] border border-red-500/30 bg-red-500/5 shadow-sm">
        <div className="divide-y divide-red-500/20">
          {SWITCHES.map(({ key, icon, label, desc }) => {
            ...
            return (
              <div key={key} className={cn('flex items-center justify-between p-5 transition-colors', isArmed ? 'bg-red-500/10' : 'hover:bg-red-500/5')}>
                <div className="flex items-start gap-4">
                  <div className="flex size-10 shrink-0 items-center justify-center rounded-[8px] bg-red-500/20 text-[20px]">{icon}</div>
                  <div>
                    <h4 className={cn('font-semibold', isArmed ? 'text-red-500' : 'text-[var(--text)]')}>{label}</h4>
                    <p className={cn('mt-1 text-[13px] leading-relaxed', isArmed ? 'text-red-400' : 'text-[var(--text2)]')}>{desc}</p>
                  </div>
                </div>
                <div className="pl-4">
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-[12px] border border-[var(--border)] bg-[var(--bg1)] p-6 shadow-xl animate-in zoom-in-95 duration-200">
            ...
            <div className="mt-4 rounded-md border border-red-500/30 bg-red-500/10 p-4">
              <p className="font-medium text-[var(--text)]">{SWITCHES.find((s) => s.key === confirmKey)?.label}</p>
              <p className="mt-1 text-[13px] text-[var(--text2)]">{SWITCHES.find((s) => s.key === confirmKey)?.desc}</p>
            </div>
            ...
            <div className="mt-6 flex justify-end gap-3">
              ...
              <Button variant="danger" className="bg-red-500 text-white hover:bg-red-600 focus-visible:ring-red-500" onClick={confirmToggle}>
                Arm Killswitch
              </Button>
            </div>
          </div>
