...
import { Check, ChevronDown, CircleAlert, Clock3, Copy, ExternalLink, FileWarning, Sparkles } from "lucide-react";
...
export function OverviewCards({ entity }: { entity: JsonRecord }) {
  ...
  return <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">{cards.map((card) => {
...
export function StructuredSections({ detail }: { detail: ObservabilityEventDetail }) {
  return <div className="flex flex-col gap-4"><StructuredDataSection title="Attributes" data={detail.attributes} /><StructuredDataSection title="Metadata" data={detail.metadata} /><StructuredDataSection title="Payload" data={detail.payload} /><StructuredDataSection title="Raw response" data={detail} /></div>;
}
...
export function InvestigationHeader({ resource, entity, detail, id }: { resource: DetailResource; entity: JsonRecord; detail: ObservabilityEventDetail; id: string }) {
  const config = resourceConfig(resource); const Icon = config.icon; const title = stringValue(pick(entity, resource === "errors" ? ["errorName", "error_name", "name", "message"] : resource === "requests" ? ["endpoint", "url", "route", "name"] : resource === "traces" ? ["rootSpanName", "root_span_name", "name"] : resource === "metrics" ? ["metricName", "metric_name"] : resource === "profiles" ? ["profileType", "profile_type"] : resource === "crons" ? ["monitorName", "monitor_name", "monitorSlug", "monitor_slug"] : ["name", "message"])) ?? config.singular; const subtitle = stringValue(pick(entity, ["message", "description", "endpoint", "route"])); const status = stringValue(pick(entity, ["status", "spanStatus", "span_status", "rootSpanStatus", "root_span_status"])); const severity = stringValue(pick(entity, ["severity", "level"])); const json = JSON.stringify(detail, null, 2); const download = () => { const blob = new Blob([json], { type: "application/json" }); const url = URL.createObjectURL(blob); const anchor = document.createElement("a"); anchor.href = url; anchor.download = `${resource}-${id}.json`; anchor.click(); URL.revokeObjectURL(url); }; const share = async () => { const shareData = { title, text: `${config.singular}: ${title}`, url: window.location.href }; if (navigator.share) await navigator.share(shareData); else await navigator.clipboard.writeText(window.location.href); };
  return <header className="rounded-[var(--radius-lg)] border border-[var(--border)] bg-[var(--bg1)] p-5"><div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between"><div className="min-w-0"><div className="mb-2 flex flex-wrap items-center gap-2"><EventTypeBadge type={resource === "crons" ? "cron_checkin" : resource.slice(0, -1)} /><EnvironmentBadge environment={stringValue(pick(entity, ["environment"]))} />{stringValue(pick(entity, ["service"])) && <UiBadge variant="secondary">{stringValue(pick(entity, ["service"]))}</UiBadge>}{status && <StatusBadge status={status} />}{severity && <SeverityBadge severity={severity} />}</div><div className="flex items-start gap-3"><div className="mt-1 flex size-10 shrink-0 items-center justify-center rounded-[var(--radius)] bg-[var(--bg2)]" style={{ color: config.accent }}><Icon className="size-5" /></div><div className="min-w-0"><h1 className="break-words font-[family-name:var(--display)] text-[26px] font-semibold tracking-[-0.025em] text-[var(--text)]">{title}</h1>{subtitle && <p className="mt-1 max-w-3xl break-words text-[13px] leading-relaxed text-[var(--text2)]">{subtitle}</p>}<div className="mt-3 flex flex-wrap items-center gap-3 text-[11px] text-[var(--text3)]"><span className="font-[family-name:var(--mono)]">{id}</span><CopyButton value={id} label="Copy ID" /><Timestamp value={pick(entity, ["occurredAt", "occurred_at", "timestamp"]) as string | number} /></div></div></div></div><div className="flex flex-wrap items-center gap-2 xl:max-w-sm xl:justify-end"><CopyButton value={json} label="Copy JSON" /><details className="relative"><summary className="flex h-7 cursor-pointer list-none items-center gap-1.5 rounded-[var(--radius)] border border-[var(--border2)] px-2.5 text-[12px] text-[var(--text2)] hover:text-[var(--text)]"><FileWarning className="size-3.5" /> Raw response <ChevronDown className="size-3" /></summary><div className="absolute right-0 top-9 z-20 w-56 rounded-[var(--radius-lg)] border border-[var(--border)] bg-[var(--bg1)] p-2 shadow-xl"><button type="button" onClick={download} className="flex w-full items-center gap-2 rounded p-2 text-left text-[12px] text-[var(--text2)] hover:bg-[var(--bg2)]"><Copy className="size-3.5" />Download JSON</button></div></details><Button variant="outline" onClick={() => void share()}>Share</Button></div></div></header>;
}
