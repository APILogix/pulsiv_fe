...
import type { RequestDeveloperToolsResponse } from "./types";

export function DeveloperToolsSection({ publicId }: { publicId: string }) {
  const activeOrgId = useOrgStore((state) => state.activeOrgId);
  const [open, setOpen] = useState(false);
  const [data, setData] = useState<RequestDeveloperToolsResponse | null>(null);
  ...
  useEffect(() => {
    ...
    apiClient
      .get(`/organizations/${activeOrgId}/observability/requests/${encodeURIComponent(publicId)}/developer-tools`)
      .then((res) => {
        if (cancelled) return;
        setData((res.data?.data || res.data) as RequestDeveloperToolsResponse);
      })
      .catch((err: unknown) => {
        if (cancelled) return;
        setError(err instanceof Error ? err.message : "Failed to load developer tools");
      })
      .finally(() => {
  ...
  return (
    ...
    >
      {!open ? (
        <EmptyInline message="Collapsed by default to keep the detail payload small. Expand to load raw SDK, normalized, and database records." />
      ) : loading ? (
